...
export default function CategorySelect({ onPick }) {
  ...
  return (
    <div className="px-5 sm:px-8 pt-6 pb-16 max-w-3xl mx-auto">
      <h2 className="font-display text-3xl text-shell-100">{t('category.title')}</h2>
      <p className="mt-2 text-shell-300/70">
        {t('category.subtitle')}
      </p>

      <div className="mt-6 flex flex-wrap gap-4">
        <div role="radiogroup" aria-label={t('category.modeLabel')} className="flex rounded-full border border-shell-300/15 p-1">
          {[
        ...
        <div role="radiogroup" aria-label={t('category.difficultyLabel')} className="flex rounded-full border border-shell-300/15 p-1">
          {[{ id: 'all', label: t('category.difficultyAll') }, ...DIFFICULTIES.map((d) => ({ ...d, label: t(`difficulties.${d.id}`) }))].map((d) => (
            <button
              key={d.id}
              role="radio"
      ...
      >
        <div>
          <p className="font-display text-lg text-turmeric-300">{t('common.allCategories')}</p>
          <p className="text-sm text-shell-300/60 mt-0.5">{t('category.allCategoriesBlurb')}</p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <p className="font-display text-lg text-shell-100">{t(`categories.${cat.id}.label`)}</p>
                <span className="font-mono text-[11px] text-shell-300/40">{available}</span>
              </div>
              <p className="text-sm text-shell-300/60 mt-0.5">{t(`categories.${cat.id}.tagline`)}</p>
            </button>
          )
        })}
      </div>
